...
class ModelBase(metaclass=ABCMeta):
    ...
    def load_training_data(self):
        data_filepath = data_utilities.get_processed_data_filepath(f'{self.experiment_name}_data.parquet')
        if self.rerun_sql:
            df = self.save_training_data_to_file(conn=data_utilities.initialize_db(), data_filepath=data_filepath)
        else:
            df = pd.read_parquet(data_filepath)
        self.features = [col for col in df.columns if col not in self.unused_cols and col != self.target]
    ...
    def get_most_recent_run(self):
        experiment_model_dir = os.path.join(data_utilities.path_to_models, self.experiment_name)
        model_dirs = [os.path.join(experiment_model_dir, d) for d in os.listdir(experiment_model_dir) if os.path.isdir(os.path.join(experiment_model_dir, d))]
    ...
    def calculate_metrics(self, df_test, count_players, suffix=''):
        metrics = {}
        metrics[f'kendall_tau_correlation{suffix}'] = df_test[self.pred_column].corr(df_test['adjusted_points'], method='kendall')
        metrics[f'spearman_rho_correlation{suffix}'] = df_test[self.pred_column].corr(df_test['adjusted_points'], method='spearman')
        sum = 0
        num_hits = 0
        for round in df_test['round'].unique():
            df_round = df_test.loc[df_test['round'] == round].sort_values(self.pred_column, ascending=False).head(count_players)
        ...
        metrics[f'top_{count_players}_avg{suffix}'] = (sum / (num_testing_weeks * count_players))
        metrics[f'top_{count_players}_hit_rate{suffix}'] = (num_hits / (num_testing_weeks * count_players))
        return metrics
    ...
    def get_experiment_info(self, model, df_test):
        ...
        for position_id, count_players in position_counts.items():
            ...
            metrics.update(self.calculate_metrics(df_position, count_players, f'.position.{position_id}'))
        artifacts = {}
        #tags = {
    ...
    def log_experiment(self, model, df_test):
        ...
        mlflow_uri = os.path.join('file:/' + data_utilities.get_project_directory(), 'mlruns')
    ...
    def evaluate_model(self, df_train, df_test, df_valid):
        ...
        for test_block in tqdm(sorted(df_test['unique_round'].unique())):
            ...
            model = self.train_model(input_x_dicts['train'], input_y_dicts['train'], input_x_dicts['valid'], input_y_dicts['valid'])

            df_predictions_fold = self.predict(model, input_x_dicts['test'])
            df_predictions_fold = pd.concat([df_test_fold[['player_id', 'player_name', 'opponent', 'round', 'position_id', 'adjusted_points']].reset_index(drop=True), df_predictions_fold.reset_index(drop=True)], axis=1)
        ...
        df_predictions.to_csv(data_utilities.get_processed_data_filepath(f'{self.experiment_name}_eval_predictions.csv'), index=False)
    ...
    def generate_current_predictions(self, df_train, df_test, df_valid, df_new, run_id):
        ...
        df_predictions = pd.concat([df_new[['player_id', 'player_name', 'opponent', 'round', 'position_id', 'cost']].reset_index(drop=True), df_predictions.reset_index(drop=True)], axis=1)
        df_predictions.to_csv(data_utilities.get_processed_data_filepath(f'{self.experiment_name}_current_predictions.csv'), index=False)
