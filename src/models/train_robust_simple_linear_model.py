...
class Robust_Simple_Linear_Model(ModelBase):
    ...
    def save_training_data_to_file(self, conn, data_filepath):
        ...
        advanced_position = pd.read_sql_query('SELECT player_id, season, advanced_position from advanced_position', conn)
        ...
        df = pd.merge(df, advanced_position, how='left', on=['player_id', 'season'], suffixes=('', '_y'))
        df = df.dropna(subset=['position_id'])
        df['last_6_avg'] = df['last_6_avg'].fillna(df['last_6_avg'].mean())
        df['advanced_position+opponent'] = df['opponent'] + '_' + df['advanced_position']
        ...
        dummy_cols = ['team', 'opponent', 'advanced_position', 'advanced_position+opponent', 'home+opponent', 'home+team']
    ...
    def prepare_x_input_dicts(self, df_dict):
        ...
        df_train['sample_weight'] = np.sqrt(df_train['unique_round']/min_important_round)
    ...
    def train_model(self, X_train, y_train, X_valid, y_valid):
        ...
        model = model.fit(X_train[self.features], y_train, sample_weight=X_train['sample_weight'].values)
        return model


    def load_model(self, run_id):
        model_path = os.path.join(data_utilities.get_model_filepath(self.experiment_name, str(run_id)))
    ...
    def save_model(self, model, run_id):
        model_path = os.path.join(data_utilities.get_model_filepath(self.experiment_name, str(run_id)))
        mlflow.sklearn.save_model(model, model_path, serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE)  
...
if __name__ == '__main__':
    ...
    model = Robust_Simple_Linear_Model(parameters, target, unused_cols, rerun_sql=rerun_sql)
