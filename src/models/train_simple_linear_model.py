...
class Simple_Linear_Model(ModelBase):
    ...
    def load_model(self, run_id):
        model = mlflow.h2o.load_model(data_utilities.get_model_filepath(self.experiment_name, str(run_id)))
        return model


    def save_model(self, model, run_id):
        mlflow.h2o.save_model(model, data_utilities.get_model_filepath(self.experiment_name, str(run_id)))  
        df_coef = model._model_json['output']['coefficients_table'].as_data_frame()
        df_coef.to_csv(data_utilities.get_processed_data_filepath(f'{self.experiment_name}_coef.csv'), index=False)
...
if __name__ == '__main__':
    ...
    unused_cols = ['event_id', 'player_id', 'player_name', 'unique_round', 'cost', 'dataset', 'season', 'round', 'position_id']
